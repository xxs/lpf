...
public class MaterialPriceServlet extends HttpServlet {
		...
		try {
			...
			try {
				for (int i = 1; i < sheet.getRows(); i++) {
					...
					oldcostprice = cs.getContents();
					Cell c12 = sheet.getCell(4, i);
					costprice = c12.getContents();

					if (!"".equals(prod_nc_pk)) {
						rs = stmt.executeQuery("select count(0) con from sn_product_price  where prod_nc_pk = '"+prod_nc_pk+"' and startdate = '"+startdate+"'");
						while(rs.next()){
							count = rs.getInt("con");
						}
							if(count<=0){
								sql = "insert into SN_PRODUCT_PRICE (PROD_NC_PK, OLDCOSTPRICE, COSTPRICE, REMARK, MAKERID, STARTDATE)";
								sql += " values('" + prod_nc_pk + "','" + oldcostprice
