...
public class DayZLNCtoBQSenderBySoApply extends BaseDao implements Runnable {
	...
	public void NCtoBQ() throws Exception {
		...
		try {
			...
			sql.append("  where ay.dbilldate >=to_char((sysdate - ").append(this.getDays()+"),'yyyy-mm-dd')");
			sql.append("  and substr(ay.ts,1,10)=to_char((sysdate - ").append(this.getBeforedays()+"),'yyyy-mm-dd')");
