...
public class DayZLNCtoBQSenderBySoPreorderB extends BaseDao implements Runnable {
	...
	private static Log log=LogFactory.getLog(DayZLNCtoBQSenderBySoPreorderB.class);
	...
	public void NCtoBQ() throws Exception {
		...
		try {
			...
			log.info("预订单辅表增量数据抽取完毕,查询NC并插入BQ数据库的行数为："+tm+"。开始时间为："+begindate+";结束时间为"+new Timestamp(new Date().getTime()));
			System.out.println("预订单辅表增量数据抽取完毕");
			System.out.println("预订单辅表增量数据抽取完毕,查询NC并插入BQ数据库的行数为："+tm+"。开始时间为："+begindate+";结束时间为"+new Timestamp(new Date().getTime()));
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
