...
public class NCtoBQSenderBySoSale extends BaseDao implements Runnable {

	public NCtoBQSenderBySoSale() {
		System.out.println("NCtoBQSenderBySoSale 无参构造函数");
	}
	...
	public void run() {
		try {
			DateLoop("2012-10-01", "2012-11-28",5);
	...
	public void DateLoop(String begindate,String enddate,int days) throws Exception{
		...
		System.out.println("订单主表时间区间："+sdf.format(date1)+" to "+sdf.format(datetemp));
		...
		while(mm.dateCompare(datetemp , date2)){
			...
			if(mm.dateCompare(date2  ,datetemp )){
				System.out.println("订单主表时间区间："+sdf.format(mm.getDateAfterDay(datetemp, -days))+" to "+sdf.format(date2));
				NCtoBQ(sdf.format(mm.getDateAfterDay(datetemp, -days)), sdf.format(date2));
			}else{
				System.out.println("订单主表时间区间："+sdf.format(mm.getDateAfterDay(datetemp, -days))+" to "+sdf.format(datetemp));
	...
	public void NCtoBQ(String beginDate,String endDate) throws Exception {
		...
		try {
			...
			while(restNC.next()){
				...
				insetSql.append("insert into DWT_SO_SALE ( 				  ");
